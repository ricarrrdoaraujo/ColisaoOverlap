...
void APickUpAmmo::ColidiuComPickUp(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if ((OtherActor != nullptr) && (OtherActor != this) && (OtherComp != nullptr))
	{
		...
		//OverlappedComp é o componente deste ator (APickUpAmmo) que sofreu overlap
		//Neste caso OverlappedComponent = Colisor
		UE_LOG(LogTemp, Warning, TEXT("OtherActor = %s "), *OtherActor->GetName());
		//OtherActor é o ator que se sobrepoe a este ator(APickUpAmmo)
		UE_LOG(LogTemp, Error, TEXT("OtherComp = %s "), *OtherComp->GetName());
		//OtherComp é o componente do outro ator e que fez overlap com este ator
		...
		//Indica que o som nao irá parar caso o proprietario deste componente seja destruido
